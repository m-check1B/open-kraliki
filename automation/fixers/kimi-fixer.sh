#!/bin/bash
# kimi-fixer.sh — AI Fixer using Kimi Code CLI (Moonshot)
# Picks up issues from Linear, uses Kimi Code CLI to fix code, commits + pushes.
#
...
LOGDIR="$HOME/logs/kimi-fixer"
FIXER_NAME="Kimi Fixer"
FIXER_SLOT="3"
CLI_COMMAND="kimi"
...
LOCKFILE="${LOGDIR}/kimi-fixer.lock"
...
if [ -f "$LOCKFILE" ]; then
  ...
  if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null; then
    echo "Another kimi-fixer is running (PID ${LOCK_PID}), skipping." | tee -a "$LOGFILE"
    exit 0
  else
...
for i in $(seq 0 $((ISSUE_COUNT - 1))); do
  ...
  # ── Call Kimi Code to fix the code ──────────────────────────────
  echo "Calling ${CLI_COMMAND} to fix..." | tee -a "$LOGFILE"

  TMPFILE_OUT=$(mktemp)

  set -m
  ( cd "$PROJECT_DIR" && echo "$PROMPT" | kimi --yes 2>&1 ) > "$TMPFILE_OUT" 2>&1 &
  CLI_PID=$!
...
")
  ...
  # ── Git commit + push ──────────────────────────────────────────
  COMMIT_MSG="${COMMIT_PREFIX} Fix (kimi): ${ISSUE_TITLE}"
