#!/bin/bash
# gemini-fixer.sh — AI Fixer using Gemini CLI
# Picks up issues from Linear, uses Gemini CLI to fix code, commits + pushes.
#
...
LOGDIR="$HOME/logs/gemini-fixer"
FIXER_NAME="Gemini Fixer"
FIXER_SLOT="3"
CLI_COMMAND="gemini"
...
LOCKFILE="${LOGDIR}/gemini-fixer.lock"
...
if [ -f "$LOCKFILE" ]; then
  ...
  if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null; then
    echo "Another gemini-fixer is running (PID ${LOCK_PID}), skipping." | tee -a "$LOGFILE"
    exit 0
  else
...
for i in $(seq 0 $((ISSUE_COUNT - 1))); do
  ...
  set -m
  ( cd "$PROJECT_DIR" && gemini -p "$PROMPT" --yolo --sandbox false ) > "$TMPFILE_OUT" 2>&1 &
  CLI_PID=$!
...
")
  ...
  # ── Git commit + push ──────────────────────────────────────────
  COMMIT_MSG="${COMMIT_PREFIX} Fix (gemini): ${ISSUE_TITLE}"
