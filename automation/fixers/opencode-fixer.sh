#!/bin/bash
# opencode-fixer.sh — AI Fixer using Opencode CLI
# Picks up issues from Linear, uses Opencode CLI to fix code, commits + pushes.
#
...
LOGDIR="$HOME/logs/opencode-fixer"
FIXER_NAME="Opencode Fixer"
FIXER_SLOT="2"
...
LOCKFILE="${LOGDIR}/opencode-fixer.lock"
...
if [ -f "$LOCKFILE" ]; then
  ...
  if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null; then
    echo "Another opencode-fixer is running (PID ${LOCK_PID}), skipping." | tee -a "$LOGFILE"
    exit 0
  else
...
for i in $(seq 0 $((ISSUE_COUNT - 1))); do
  ...
  set -m
  ( cd "$PROJECT_DIR" && opencode run --dir "$PROJECT_DIR" --agent build "$(cat "$TMPFILE_IN")" ) > "$TMPFILE_OUT" 2>&1 &
  CLI_PID=$!
...
")
  ...
  # ── Git commit + push ──────────────────────────────────────────
  COMMIT_MSG="${COMMIT_PREFIX} Fix (opencode): ${ISSUE_TITLE}"
