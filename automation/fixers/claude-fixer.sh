#!/bin/bash
# claude-fixer.sh — AI Fixer using Claude CLI
# Picks up issues from Linear, uses Claude CLI to fix code, commits + pushes.
#
...
if [ -f "$LOCKFILE" ]; then
  ...
  if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null; then
    echo "Another claude-fixer is running (PID ${LOCK_PID}), skipping." | tee -a "$LOGFILE"
    exit 0
  else
...
for i in $(seq 0 $((ISSUE_COUNT - 1))); do
  ...
  # Run CLI in a new process group so we can kill the whole tree
...
")
  ...
  # ── Git commit + push ──────────────────────────────────────────
  COMMIT_MSG="${COMMIT_PREFIX} Fix: ${ISSUE_TITLE}"
  ...
  # Extract last line of CLI output as summary (the "Fixed: ..." line)
